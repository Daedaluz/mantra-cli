...
var auditUserCmd = &cobra.Command{
	...
	Short: "Audit authentication history for a user",
...
func init() {
	adminCmd.AddCommand(auditUserCmd)
	auditUserCmd.Flags().StringP("domain", "d", envStr("DOMAIN", ""), "domain to audit [$DOMAIN]")
	auditUserCmd.Flags().String("from", "", "filter entries from this time (RFC3339)")
	auditUserCmd.Flags().String("to", "", "filter entries to this time (RFC3339)")
	auditUserCmd.Flags().BoolP("verbose", "v", false, "show detailed entry information")
}
